...
var fV_REKAM_MEDISadd;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "add";
    fV_REKAM_MEDISadd = currentForm = new ew.Form("fV_REKAM_MEDISadd", "add");
    ...
    // Validate form
    fV_REKAM_MEDISadd.validate = function () {
    ...
    // Form_CustomValidate
    fV_REKAM_MEDISadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fV_REKAM_MEDISadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fV_REKAM_MEDISadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fV_REKAM_MEDISadd" id="fV_REKAM_MEDISadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="<?= $Page->TRANS_ID->getInputTextType() ?>" data-table="V_REKAM_MEDIS" data-field="x_TRANS_ID" data-page="1" name="x_TRANS_ID" id="x_TRANS_ID" size="30" maxlength="50" placeholder="<?= HtmlEncode($Page->TRANS_ID->getPlaceHolder()) ?>" value="<?= $Page->TRANS_ID->EditValue ?>"<?= $Page->TRANS_ID->editAttributes() ?> aria-describedby="x_TRANS_ID_help">
