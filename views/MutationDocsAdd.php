...
loadjs.ready("head", function () {
    ...
    fMUTATION_DOCSadd.addFields([
        ...
        ["MUTATION_BY", [fields.MUTATION_BY.visible && fields.MUTATION_BY.required ? ew.Validators.required(fields.MUTATION_BY.caption) : null], fields.MUTATION_BY.isInvalid],
    ...
    fMUTATION_DOCSadd.lists.COMPANY_ID = <?= $Page->COMPANY_ID->toClientList($Page) ?>;
...
<div class="ew-add-div"><!-- page* -->
    <span id="el_MUTATION_DOCS_ORG_UNIT_CODE">
    <input type="hidden" data-table="MUTATION_DOCS" data-field="x_ORG_UNIT_CODE" data-hidden="1" data-page="1" name="x_ORG_UNIT_CODE" id="x_ORG_UNIT_CODE" value="<?= HtmlEncode($Page->ORG_UNIT_CODE->CurrentValue) ?>">
    </span>
<?php if ($Page->ORG_ID->Visible) { // ORG_ID ?>
    <div id="r_ORG_ID" class="form-group row">
        <label id="elh_MUTATION_DOCS_ORG_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->ORG_ID->caption() ?><?= $Page->ORG_ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php $Page->CLINIC_ID->EditAttrs->prepend("onchange", "ew.autoFill(this);"); ?>
    <select
        id="x_CLINIC_ID"
        name="x_CLINIC_ID"
...
<?php $Page->CLINIC_ID_TO->EditAttrs->prepend("onchange", "ew.autoFill(this);"); ?>
    <select
        id="x_CLINIC_ID_TO"
        name="x_CLINIC_ID_TO"
...
<?php } ?>
<?php if ($Page->MUTATION_BY->Visible) { // MUTATION_BY ?>
    <div id="r_MUTATION_BY" class="form-group row">
        <label id="elh_MUTATION_DOCS_MUTATION_BY" for="x_MUTATION_BY" class="<?= $Page->LeftColumnClass ?>"><?= $Page->MUTATION_BY->caption() ?><?= $Page->MUTATION_BY->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->MUTATION_BY->cellAttributes() ?>>
<span id="el_MUTATION_DOCS_MUTATION_BY">
<input type="<?= $Page->MUTATION_BY->getInputTextType() ?>" data-table="MUTATION_DOCS" data-field="x_MUTATION_BY" data-page="1" name="x_MUTATION_BY" id="x_MUTATION_BY" size="30" maxlength="100" placeholder="<?= HtmlEncode($Page->MUTATION_BY->getPlaceHolder()) ?>" value="<?= $Page->MUTATION_BY->EditValue ?>"<?= $Page->MUTATION_BY->editAttributes() ?> aria-describedby="x_MUTATION_BY_help">
<?= $Page->MUTATION_BY->getCustomMessage() ?>
<div class="invalid-feedback"><?= $Page->MUTATION_BY->getErrorMessage() ?></div>
</span>
</div></div>
    </div>
<?php } ?>
...
<?php if ($Page->ACKNOWLEDGEBY->Visible) { // ACKNOWLEDGEBY ?>
    <div id="r_ACKNOWLEDGEBY" class="form-group row">
        <label id="elh_MUTATION_DOCS_ACKNOWLEDGEBY" for="x_ACKNOWLEDGEBY" class="<?= $Page->LeftColumnClass ?>"><?= $Page->ACKNOWLEDGEBY->caption() ?><?= $Page->ACKNOWLEDGEBY->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->ACKNOWLEDGEBY->cellAttributes() ?>>
<span id="el_MUTATION_DOCS_ACKNOWLEDGEBY">
<input type="<?= $Page->ACKNOWLEDGEBY->getInputTextType() ?>" data-table="MUTATION_DOCS" data-field="x_ACKNOWLEDGEBY" data-page="1" name="x_ACKNOWLEDGEBY" id="x_ACKNOWLEDGEBY" size="30" maxlength="100" placeholder="<?= HtmlEncode($Page->ACKNOWLEDGEBY->getPlaceHolder()) ?>" value="<?= $Page->ACKNOWLEDGEBY->EditValue ?>"<?= $Page->ACKNOWLEDGEBY->editAttributes() ?> aria-describedby="x_ACKNOWLEDGEBY_help">
...
<?php if ($Page->COMPANY_ID->Visible) { // COMPANY_ID ?>
    <div id="r_COMPANY_ID" class="form-group row">
        <label id="elh_MUTATION_DOCS_COMPANY_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->COMPANY_ID->caption() ?><?= $Page->COMPANY_ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->COMPANY_ID->cellAttributes() ?>>
<span id="el_MUTATION_DOCS_COMPANY_ID">
<template id="tp_x_COMPANY_ID">
    <div class="custom-control custom-checkbox">
        <input type="checkbox" class="custom-control-input" data-table="MUTATION_DOCS" data-field="x_COMPANY_ID" name="x_COMPANY_ID" id="x_COMPANY_ID"<?= $Page->COMPANY_ID->editAttributes() ?>>
        <label class="custom-control-label"></label>
    </div>
</template>
<div id="dsl_x_COMPANY_ID" class="ew-item-list"></div>
<input type="hidden"
    is="selection-list"
    id="x_COMPANY_ID[]"
    name="x_COMPANY_ID[]"
    value="<?= HtmlEncode($Page->COMPANY_ID->CurrentValue) ?>"
    data-type="select-multiple"
    data-template="tp_x_COMPANY_ID"
    data-target="dsl_x_COMPANY_ID"
    data-repeatcolumn="5"
    class="form-control<?= $Page->COMPANY_ID->isInvalidClass() ?>"
    data-table="MUTATION_DOCS"
...
<?php
    if (in_array("GOOD_GF", explode(",", $Page->getCurrentDetailTable())) && $GOOD_GF->DetailAdd) {
?>
