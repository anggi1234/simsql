...
// Page object
$AntrianPoliEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fANTRIAN_POLIedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fANTRIAN_POLIedit = currentForm = new ew.Form("fANTRIAN_POLIedit", "edit");

    // Add fields
    var currentTable = <?= JsonEncode(GetClientVar("tables", "ANTRIAN_POLI")) ?>,
        fields = currentTable.fields;
    if (!ew.vars.tables.ANTRIAN_POLI)
        ew.vars.tables.ANTRIAN_POLI = currentTable;
    fANTRIAN_POLIedit.addFields([
    ...
    // Validate form
    fANTRIAN_POLIedit.validate = function () {
    ...
    // Form_CustomValidate
    fANTRIAN_POLIedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fANTRIAN_POLIedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fANTRIAN_POLIedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fANTRIAN_POLIedit" id="fANTRIAN_POLIedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->Id->Visible) { // Id ?>
    <div id="r_Id" class="form-group row">
        <label id="elh_ANTRIAN_POLI_Id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Id->caption() ?><?= $Page->Id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->Id->cellAttributes() ?>>
<span id="el_ANTRIAN_POLI_Id">
<input type="<?= $Page->Id->getInputTextType() ?>" data-table="ANTRIAN_POLI" data-field="x_Id" name="x_Id" id="x_Id" placeholder="<?= HtmlEncode($Page->Id->getPlaceHolder()) ?>" value="<?= $Page->Id->EditValue ?>"<?= $Page->Id->editAttributes() ?> aria-describedby="x_Id_help">
...
<?php if ($Page->no_urut->Visible) { // no_urut ?>
    <div id="r_no_urut" class="form-group row">
        <label id="elh_ANTRIAN_POLI_no_urut" for="x_no_urut" class="<?= $Page->LeftColumnClass ?>"><?= $Page->no_urut->caption() ?><?= $Page->no_urut->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->no_urut->cellAttributes() ?>>
<span id="el_ANTRIAN_POLI_no_urut">
<input type="<?= $Page->no_urut->getInputTextType() ?>" data-table="ANTRIAN_POLI" data-field="x_no_urut" name="x_no_urut" id="x_no_urut" size="30" placeholder="<?= HtmlEncode($Page->no_urut->getPlaceHolder()) ?>" value="<?= $Page->no_urut->EditValue ?>"<?= $Page->no_urut->editAttributes() ?> aria-describedby="x_no_urut_help">
...
<script>
loadjs.ready(["fANTRIAN_POLIedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fANTRIAN_POLIedit", "x_tanggal_daftar", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fANTRIAN_POLIedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fANTRIAN_POLIedit", "x_tanggal_panggil", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->loket->Visible) { // loket ?>
    <div id="r_loket" class="form-group row">
        <label id="elh_ANTRIAN_POLI_loket" for="x_loket" class="<?= $Page->LeftColumnClass ?>"><?= $Page->loket->caption() ?><?= $Page->loket->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->loket->cellAttributes() ?>>
<span id="el_ANTRIAN_POLI_loket">
<input type="<?= $Page->loket->getInputTextType() ?>" data-table="ANTRIAN_POLI" data-field="x_loket" name="x_loket" id="x_loket" size="30" maxlength="10" placeholder="<?= HtmlEncode($Page->loket->getPlaceHolder()) ?>" value="<?= $Page->loket->EditValue ?>"<?= $Page->loket->editAttributes() ?> aria-describedby="x_loket_help">
...
<?php if ($Page->THENAME->Visible) { // THENAME ?>
    <div id="r_THENAME" class="form-group row">
        <label id="elh_ANTRIAN_POLI_THENAME" for="x_THENAME" class="<?= $Page->LeftColumnClass ?>"><?= $Page->THENAME->caption() ?><?= $Page->THENAME->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->THENAME->cellAttributes() ?>>
<span id="el_ANTRIAN_POLI_THENAME">
<input type="<?= $Page->THENAME->getInputTextType() ?>" data-table="ANTRIAN_POLI" data-field="x_THENAME" name="x_THENAME" id="x_THENAME" size="30" maxlength="150" placeholder="<?= HtmlEncode($Page->THENAME->getPlaceHolder()) ?>" value="<?= $Page->THENAME->EditValue ?>"<?= $Page->THENAME->editAttributes() ?> aria-describedby="x_THENAME_help">
...
<?php if ($Page->VISIT_ID->Visible) { // VISIT_ID ?>
    <div id="r_VISIT_ID" class="form-group row">
        <label id="elh_ANTRIAN_POLI_VISIT_ID" for="x_VISIT_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->VISIT_ID->caption() ?><?= $Page->VISIT_ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->VISIT_ID->cellAttributes() ?>>
<input type="<?= $Page->VISIT_ID->getInputTextType() ?>" data-table="ANTRIAN_POLI" data-field="x_VISIT_ID" name="x_VISIT_ID" id="x_VISIT_ID" size="30" maxlength="50" placeholder="<?= HtmlEncode($Page->VISIT_ID->getPlaceHolder()) ?>" value="<?= $Page->VISIT_ID->EditValue ?>"<?= $Page->VISIT_ID->editAttributes() ?> aria-describedby="x_VISIT_ID_help">
<?= $Page->VISIT_ID->getCustomMessage() ?>
<div class="invalid-feedback"><?= $Page->VISIT_ID->getErrorMessage() ?></div>
<input type="hidden" data-table="ANTRIAN_POLI" data-field="x_VISIT_ID" data-hidden="1" name="o_VISIT_ID" id="o_VISIT_ID" value="<?= HtmlEncode($Page->VISIT_ID->OldValue ?? $Page->VISIT_ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
</div><!-- /page* -->
