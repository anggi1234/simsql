...
// Page object
$ItemConditionEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fITEM_CONDITIONedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fITEM_CONDITIONedit = currentForm = new ew.Form("fITEM_CONDITIONedit", "edit");
    ...
    // Validate form
    fITEM_CONDITIONedit.validate = function () {
    ...
    // Form_CustomValidate
    fITEM_CONDITIONedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fITEM_CONDITIONedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fITEM_CONDITIONedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fITEM_CONDITIONedit" id="fITEM_CONDITIONedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="ITEM_CONDITION" data-field="x_CONDITION" data-hidden="1" name="o_CONDITION" id="o_CONDITION" value="<?= HtmlEncode($Page->CONDITION->OldValue ?? $Page->CONDITION->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
