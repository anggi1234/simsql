 ...
 * Page class
 */
class AntrianPoliAdd extends AntrianPoli
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AntrianPoliAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "AntrianPoliView") {
    ...
    {
        global $Language, $Security;
        if ($this->VISIT_ID->CurrentValue != "") { // Check field with unique index
            $filter = "([VISIT_ID] = '" . AdjustSql($this->VISIT_ID->CurrentValue, $this->Dbid) . "')";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->VISIT_ID->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->VISIT_ID->CurrentValue, $idxErrMsg);
