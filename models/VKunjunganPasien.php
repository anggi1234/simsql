...
class VKunjunganPasien extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "dbo.PASIEN_VISITATION INNER JOIN dbo.TREATMENT_AKOMODASI ON dbo.PASIEN_VISITATION.VISIT_ID = dbo.TREATMENT_AKOMODASI.VISIT_ID AND dbo.PASIEN_VISITATION.TRANS_ID = dbo.TREATMENT_AKOMODASI.TRANS_ID";
        $this->Dbid = 'DB';
        ...
        // VISIT_ID
        $this->VISIT_ID = new DbField('V_KUNJUNGAN_PASIEN', 'V_KUNJUNGAN_PASIEN', 'x_VISIT_ID', 'VISIT_ID', 'dbo.PASIEN_VISITATION.VISIT_ID', 'dbo.PASIEN_VISITATION.VISIT_ID', 200, 50, -1, false, 'dbo.PASIEN_VISITATION.VISIT_ID', false, false, false, 'FORMATTED TEXT', 'HIDDEN');
        ...
        // TRANS_ID
        $this->TRANS_ID = new DbField('V_KUNJUNGAN_PASIEN', 'V_KUNJUNGAN_PASIEN', 'x_TRANS_ID', 'TRANS_ID', 'dbo.PASIEN_VISITATION.TRANS_ID', 'dbo.PASIEN_VISITATION.TRANS_ID', 200, 50, -1, false, 'dbo.PASIEN_VISITATION.TRANS_ID', false, false, false, 'FORMATTED TEXT', 'HIDDEN');
    ...
    public function getDetailUrl()
    {
        ...
        if ($this->getCurrentDetailTable() == "V_AKOMODASI_KAMAR") {
            $detailUrl = Container("V_AKOMODASI_KAMAR")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue);
            $detailUrl .= "&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue);
            $detailUrl .= "&" . GetForeignKeyUrl("fk_DIANTAR_OLEH", $this->DIANTAR_OLEH->CurrentValue);
        ...
        if ($detailUrl == "") {
            $detailUrl = "VKunjunganPasienList";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "dbo.PASIEN_VISITATION INNER JOIN dbo.TREATMENT_AKOMODASI ON dbo.PASIEN_VISITATION.VISIT_ID = dbo.TREATMENT_AKOMODASI.VISIT_ID AND dbo.PASIEN_VISITATION.TRANS_ID = dbo.TREATMENT_AKOMODASI.TRANS_ID";
    }
    ...
    {
        return $this->SqlSelect ?? $this->getQueryBuilder()->select("dbo.PASIEN_VISITATION.NO_REGISTRATION, dbo.PASIEN_VISITATION.STATUS_PASIEN_ID, dbo.PASIEN_VISITATION.GENDER, dbo.PASIEN_VISITATION.IDXDAFTAR, dbo.PASIEN_VISITATION.ISRJ, dbo.PASIEN_VISITATION.SERVED_INAP, dbo.PASIEN_VISITATION.VISIT_ID, dbo.TREATMENT_AKOMODASI.TREATMENT, dbo.TREATMENT_AKOMODASI.CLASS_ROOM_ID, dbo.TREATMENT_AKOMODASI.DOCTOR, dbo.TREATMENT_AKOMODASI.BED_ID, dbo.PASIEN_VISITATION.DIANTAR_OLEH, dbo.TREATMENT_AKOMODASI.EXIT_DATE, dbo.TREATMENT_AKOMODASI.KELUAR_ID, dbo.TREATMENT_AKOMODASI.AGEYEAR, dbo.PASIEN_VISITATION.TRANS_ID");
    }
    ...
    {
        // Cascade Update detail table 'V_AKOMODASI_KAMAR'
        $cascadeUpdate = false;
        $rscascade = [];
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("V_AKOMODASI_KAMAR")->loadRs("dbo.TREATMENT_BILL.VISIT_ID = " . QuotedValue($rsold['VISIT_ID'], DATATYPE_STRING, 'DB') . " AND " . "dbo.TREATMENT_BILL.NO_REGISTRATION = " . QuotedValue($rsold['NO_REGISTRATION'], DATATYPE_STRING, 'DB') . " AND " . "dbo.TREATMENT_BILL.THENAME = " . QuotedValue($rsold['DIANTAR_OLEH'], DATATYPE_STRING, 'DB') . " AND " . "dbo.TREATMENT_BILL.TRANS_ID = " . QuotedValue($rsold['TRANS_ID'], DATATYPE_STRING, 'DB'))->fetchAll(\PDO::FETCH_ASSOC);
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("V_AKOMODASI_KAMAR")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("V_AKOMODASI_KAMAR")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("V_AKOMODASI_KAMAR")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'V_AKOMODASI_KAMAR'
        $dtlrows = Container("V_AKOMODASI_KAMAR")->loadRs("dbo.TREATMENT_BILL.VISIT_ID = " . QuotedValue($rs['VISIT_ID'], DATATYPE_STRING, "DB") . " AND " . "dbo.TREATMENT_BILL.NO_REGISTRATION = " . QuotedValue($rs['NO_REGISTRATION'], DATATYPE_STRING, "DB") . " AND " . "dbo.TREATMENT_BILL.THENAME = " . QuotedValue($rs['DIANTAR_OLEH'], DATATYPE_STRING, "DB") . " AND " . "dbo.TREATMENT_BILL.TRANS_ID = " . QuotedValue($rs['TRANS_ID'], DATATYPE_STRING, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("V_AKOMODASI_KAMAR")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("V_AKOMODASI_KAMAR")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("V_AKOMODASI_KAMAR")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function getEditUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("VKunjunganPasienEdit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("VKunjunganPasienAdd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        $curVal = trim(strval($this->CLASS_ROOM_ID->CurrentValue));
        if ($curVal != "") {
            $this->CLASS_ROOM_ID->EditValue = $this->CLASS_ROOM_ID->lookupCacheOption($curVal);
            if ($this->CLASS_ROOM_ID->EditValue === null) { // Lookup from database
                $filterWrk = "[CLASS_ROOM_ID]" . SearchString("=", $curVal, DATATYPE_STRING, "");
                $sqlWrk = $this->CLASS_ROOM_ID->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->CLASS_ROOM_ID->Lookup->renderViewRow($rswrk[0]);
                    $this->CLASS_ROOM_ID->EditValue = $this->CLASS_ROOM_ID->displayValue($arwrk);
                } else {
                    $this->CLASS_ROOM_ID->EditValue = $this->CLASS_ROOM_ID->CurrentValue;
                }
            }
        } else {
            $this->CLASS_ROOM_ID->EditValue = null;
        }
        $this->CLASS_ROOM_ID->ViewCustomAttributes = "";
