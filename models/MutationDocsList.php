...
class MutationDocsList extends MutationDocs
{
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "MutationDocsAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->ORG_ID_TO->AdvancedSearch->toJson(), ","); // Field ORG_ID_TO
    ...
    protected function restoreFilterList()
    {
        ...
        // Field ORG_ID_TO
        $this->ORG_ID_TO->AdvancedSearch->SearchValue = @$filter["x_ORG_ID_TO"];
        $this->ORG_ID_TO->AdvancedSearch->SearchOperator = @$filter["z_ORG_ID_TO"];
        $this->ORG_ID_TO->AdvancedSearch->SearchCondition = @$filter["v_ORG_ID_TO"];
        $this->ORG_ID_TO->AdvancedSearch->SearchValue2 = @$filter["y_ORG_ID_TO"];
        $this->ORG_ID_TO->AdvancedSearch->SearchOperator2 = @$filter["w_ORG_ID_TO"];
        $this->ORG_ID_TO->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->MUTATION_BY); // MUTATION_BY
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_GOOD_GF"
        $item = &$this->ListOptions->add("detail_GOOD_GF");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'GOOD_GF') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        // "detail_GOOD_GF"
        $opt = $this->ListOptions["detail_GOOD_GF"];
        if ($Security->allowList(CurrentProjectID() . 'GOOD_GF')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("GOOD_GF", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("GoodGfList?" . Config("TABLE_SHOW_MASTER") . "=MUTATION_DOCS&" . GetForeignKeyUrl("fk_CLINIC_ID_TO", $this->CLINIC_ID_TO->CurrentValue) . "&" . GetForeignKeyUrl("fk_CLINIC_ID_TO", $this->CLINIC_ID_TO->CurrentValue) . "&" . GetForeignKeyUrl("fk_CLINIC_ID", $this->CLINIC_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_DOC_NO", $this->DOC_NO->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("GoodGfGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'MUTATION_DOCS')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=GOOD_GF");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "GOOD_GF";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'MUTATION_DOCS')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=GOOD_GF");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
                $detailEditTblVar .= "GOOD_GF";
            }
            if ($links != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                $item = &$option->add("detailadd_GOOD_GF");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=GOOD_GF");
                $detailPage = Container("GoodGfGrid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'MUTATION_DOCS') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
                    $detailTableLink .= "GOOD_GF";
                }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fMUTATION_DOCSlist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fMUTATION_DOCSlist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fMUTATION_DOCSlist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_MUTATION_DOCS" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_MUTATION_DOCS\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fMUTATION_DOCSlist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
