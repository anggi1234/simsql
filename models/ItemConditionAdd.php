 ...
 * Page class
 */
class ItemConditionAdd extends ItemCondition
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ItemConditionAdd";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("CONDITION") ?? Route("CONDITION")) !== null) {
                $this->CONDITION->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("ItemConditionList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ItemConditionList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ItemConditionView") {
    ...
    {
        global $Language, $Security;
        if ($this->CONDITION->CurrentValue != "") { // Check field with unique index
            $filter = "([CONDITION] = " . AdjustSql($this->CONDITION->CurrentValue, $this->Dbid) . ")";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->CONDITION->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->CONDITION->CurrentValue, $idxErrMsg);
        ...
        // CONDITION
        $this->CONDITION->setDbValueDef($rsnew, $this->CONDITION->CurrentValue, 0, false);
        ...
        if ($insertRow && $this->ValidateKey && strval($rsnew['CONDITION']) == "") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ItemConditionList"), "", $this->TableVar, true);
