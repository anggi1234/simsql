...
class TreatmentBill extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // ORG_UNIT_CODE
        $this->ORG_UNIT_CODE = new DbField('TREATMENT_BILL', 'TREATMENT_BILL', 'x_ORG_UNIT_CODE', 'ORG_UNIT_CODE', '[ORG_UNIT_CODE]', '[ORG_UNIT_CODE]', 200, 50, -1, false, '[ORG_UNIT_CODE]', false, false, false, 'FORMATTED TEXT', 'HIDDEN');
        ...
        switch ($CurrentLanguage) {
            case "en":
                $this->TARIF_ID->Lookup = new Lookup('TARIF_ID', 'TREAT_TARIF', false, 'TARIF_ID', ["TARIF_NAME","AMOUNT_PAID","",""], [], [], [], [], ["TARIF_NAME","AMOUNT_PAID","AMOUNT_PAID","AMOUNT_PAID"], ["x_TREATMENT","x_AMOUNT","x_sell_price","x_TAGIHAN"], '[TARIF_NAME] ASC', '');
                break;
            default:
                $this->TARIF_ID->Lookup = new Lookup('TARIF_ID', 'TREAT_TARIF', false, 'TARIF_ID', ["TARIF_NAME","AMOUNT_PAID","",""], [], [], [], [], ["TARIF_NAME","AMOUNT_PAID","AMOUNT_PAID","AMOUNT_PAID"], ["x_TREATMENT","x_AMOUNT","x_sell_price","x_TAGIHAN"], '[TARIF_NAME] ASC', '');
                break;
        ...
        // ISRJ
        $this->ISRJ = new DbField('TREATMENT_BILL', 'TREATMENT_BILL', 'x_ISRJ', 'ISRJ', '[ISRJ]', '[ISRJ]', 129, 1, -1, false, '[ISRJ]', false, false, false, 'FORMATTED TEXT', 'CHECKBOX');
        $this->ISRJ->Sortable = false; // Allow sort
        switch ($CurrentLanguage) {
            case "en":
                $this->ISRJ->Lookup = new Lookup('ISRJ', 'TREATMENT_BILL', false, '', ["","","",""], [], [], [], [], [], [], '', '');
                break;
            default:
                $this->ISRJ->Lookup = new Lookup('ISRJ', 'TREATMENT_BILL', false, '', ["","","",""], [], [], [], [], [], [], '', '');
                break;
        }
        $this->ISRJ->OptionCount = 1;
        ...
        // modified_datesys
        $this->modified_datesys = new DbField('TREATMENT_BILL', 'TREATMENT_BILL', 'x_modified_datesys', 'modified_datesys', '[modified_datesys]', CastDateFieldForLike("[modified_datesys]", 11, "DB"), 135, 8, 11, false, '[modified_datesys]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->modified_datesys->Sortable = false; // Allow sort
        $this->modified_datesys->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_SEPARATOR"], $Language->phrase("IncorrectDateDMY"));
    ...
    public function getDetailFilter()
    {
        ...
        if ($this->getCurrentMasterTable() == "PASIEN_VISITATION") {
            ...
            if ($this->NO_REGISTRATION->getSessionValue() != "") {
                $detailFilter .= " AND " . GetForeignKeySql("[NO_REGISTRATION]", $this->NO_REGISTRATION->getSessionValue(), DATATYPE_STRING, "DB");
            } else {
                return "";
            }
            if ($this->THENAME->getSessionValue() != "") {
    ...
    {
        return "[VISIT_ID]='@VISIT_ID@' AND [TRANS_ID]='@TRANS_ID@' AND [NO_REGISTRATION]='@NO_REGISTRATION@' AND [DIANTAR_OLEH]='@DIANTAR_OLEH@' AND [VISITOR_ADDRESS]='@VISITOR_ADDRESS@'";
    }
    // Detail filter
    public function sqlDetailFilter_PASIEN_VISITATION()
    {
        return "[VISIT_ID]='@VISIT_ID@' AND [TRANS_ID]='@TRANS_ID@' AND [NO_REGISTRATION]='@NO_REGISTRATION@' AND [THENAME]='@THENAME@' AND [THEADDRESS]='@THEADDRESS@'";
    }
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        $this->modified_datesys->EditValue = FormatDateTime($this->modified_datesys->CurrentValue, 11);
    ...
    public function exportDocument($doc, $recordset, $startRec = 1, $stopRec = 1, $exportPageType = "")
    {
        ...
        if ($doc->Horizontal) {
            ...
            if (!$doc->ExportCustom) {
                ...
                $doc->exportAggregate($this->MEASURE_ID, '');
                $doc->exportAggregate($this->POKOK_JUAL, '');
                $doc->exportAggregate($this->PPN, '');
                $doc->exportAggregate($this->MARGIN, '');
                $doc->exportAggregate($this->SUBSIDI, '');
                $doc->exportAggregate($this->EMBALACE, '');
                $doc->exportAggregate($this->PROFESI, '');
                $doc->exportAggregate($this->DISCOUNT, '');
                $doc->exportAggregate($this->PAY_METHOD_ID, '');
                $doc->exportAggregate($this->PAYMENT_DATE, '');
                $doc->exportAggregate($this->ISLUNAS, '');
                $doc->exportAggregate($this->DUEDATE_ANGSURAN, '');
                ...
                $doc->exportAggregate($this->RESEP_NO, '');
                $doc->exportAggregate($this->RESEP_KE, '');
                $doc->exportAggregate($this->DOSE, '');
                $doc->exportAggregate($this->ORIG_DOSE, '');
                $doc->exportAggregate($this->DOSE_PRESC, '');
                $doc->exportAggregate($this->ITER, '');
                $doc->exportAggregate($this->ITER_KE, '');
                $doc->exportAggregate($this->SOLD_STATUS, '');
                $doc->exportAggregate($this->RACIKAN, '');
                $doc->exportAggregate($this->CLASS_ROOM_ID, '');
                $doc->exportAggregate($this->KELUAR_ID, '');
                $doc->exportAggregate($this->BED_ID, '');
                $doc->exportAggregate($this->PERDA_ID, '');
                $doc->exportAggregate($this->EMPLOYEE_ID, '');
                $doc->exportAggregate($this->DESCRIPTION2, '');
                $doc->exportAggregate($this->MODIFIED_BY, '');
                $doc->exportAggregate($this->MODIFIED_DATE, '');
                $doc->exportAggregate($this->MODIFIED_FROM, '');
                $doc->exportAggregate($this->BRAND_ID, '');
                $doc->exportAggregate($this->DOCTOR, '');
                $doc->exportAggregate($this->JML_BKS, '');
                $doc->exportAggregate($this->EXIT_DATE, '');
                $doc->exportAggregate($this->FA_V, '');
                $doc->exportAggregate($this->TASK_ID, '');
                $doc->exportAggregate($this->EMPLOYEE_ID_FROM, '');
                $doc->exportAggregate($this->DOCTOR_FROM, '');
                $doc->exportAggregate($this->status_pasien_id, '');
                ...
                $doc->exportAggregate($this->serial_nb, '');
                $doc->exportAggregate($this->TREATMENT_PLAFOND, '');
                $doc->exportAggregate($this->AMOUNT_PLAFOND, '');
                $doc->exportAggregate($this->AMOUNT_PAID_PLAFOND, '');
                $doc->exportAggregate($this->CLASS_ID_PLAFOND, '');
                $doc->exportAggregate($this->PAYOR_ID, '');
                $doc->exportAggregate($this->PEMBULATAN, '');
                $doc->exportAggregate($this->ISRJ, '');
                $doc->exportAggregate($this->AGEYEAR, '');
                $doc->exportAggregate($this->AGEMONTH, '');
                $doc->exportAggregate($this->AGEDAY, '');
                $doc->exportAggregate($this->GENDER, '');
                $doc->exportAggregate($this->KAL_ID, '');
                $doc->exportAggregate($this->CORRECTION_ID, '');
                $doc->exportAggregate($this->CORRECTION_BY, '');
                $doc->exportAggregate($this->KARYAWAN, '');
                $doc->exportAggregate($this->ACCOUNT_ID, '');
                $doc->exportAggregate($this->sell_price, '');
                $doc->exportAggregate($this->diskon, '');
                $doc->exportAggregate($this->INVOICE_ID, '');
                $doc->exportAggregate($this->NUMER, '');
                $doc->exportAggregate($this->MEASURE_ID2, '');
                $doc->exportAggregate($this->POTONGAN, '');
                $doc->exportAggregate($this->BAYAR, '');
                $doc->exportAggregate($this->RETUR, '');
                $doc->exportAggregate($this->TARIF_TYPE, '');
                $doc->exportAggregate($this->PPNVALUE, '');
                $doc->exportAggregate($this->TAGIHAN, '');
                $doc->exportAggregate($this->KOREKSI, '');
                $doc->exportAggregate($this->STATUS_OBAT, '');
                $doc->exportAggregate($this->SUBSIDISAT, '');
                $doc->exportAggregate($this->PRINTQ, '');
                $doc->exportAggregate($this->PRINTED_BY, '');
                $doc->exportAggregate($this->STOCK_AVAILABLE, '');
                $doc->exportAggregate($this->STATUS_TARIF, '');
                $doc->exportAggregate($this->CLINIC_TYPE, '');
                $doc->exportAggregate($this->PACKAGE_ID, '');
                $doc->exportAggregate($this->MODULE_ID, '');
                $doc->exportAggregate($this->profession, '');
                $doc->exportAggregate($this->THEORDER, '');
                $doc->exportAggregate($this->CASHIER, '');
                $doc->exportAggregate($this->SPPFEE, '');
                $doc->exportAggregate($this->SPPBILL, '');
                $doc->exportAggregate($this->SPPRJK, '');
                $doc->exportAggregate($this->SPPJMN, '');
                $doc->exportAggregate($this->SPPKASIR, '');
                $doc->exportAggregate($this->PERUJUK, '');
                $doc->exportAggregate($this->PERUJUKFEE, '');
                $doc->exportAggregate($this->modified_datesys, '');
                $doc->exportAggregate($this->TRANS_ID, '');
                $doc->exportAggregate($this->SPPBILLDATE, '');
                $doc->exportAggregate($this->SPPBILLUSER, '');
                $doc->exportAggregate($this->SPPKASIRDATE, '');
                $doc->exportAggregate($this->SPPKASIRUSER, '');
                $doc->exportAggregate($this->SPPPOLI, '');
                $doc->exportAggregate($this->SPPPOLIUSER, '');
                $doc->exportAggregate($this->SPPPOLIDATE, '');
                $doc->exportAggregate($this->nota_temp, '');
                $doc->exportAggregate($this->CLINIC_ID_TEMP, '');
                $doc->exportAggregate($this->NOSEP, '');
