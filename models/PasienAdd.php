...
class PasienAdd extends Pasien
{
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'BLOOD_TYPE_ID' first before field var 'x_BLOOD_TYPE_ID'
        $val = $CurrentForm->hasValue("BLOOD_TYPE_ID") ? $CurrentForm->getValue("BLOOD_TYPE_ID") : $CurrentForm->getValue("x_BLOOD_TYPE_ID");
        if (!$this->BLOOD_TYPE_ID->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->BLOOD_TYPE_ID->Visible = false; // Disable update for API request
            } else {
                $this->BLOOD_TYPE_ID->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->BLOOD_TYPE_ID->CurrentValue = $this->BLOOD_TYPE_ID->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                $this->BLOOD_TYPE_ID->ViewValue = $this->BLOOD_TYPE_ID->Lookup !== null && is_array($this->BLOOD_TYPE_ID->Lookup->Options) ? $curVal : null;
            }
            if ($this->BLOOD_TYPE_ID->ViewValue !== null) { // Load from cache
                $this->BLOOD_TYPE_ID->EditValue = array_values($this->BLOOD_TYPE_ID->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[BLOOD_TYPE_ID]" . SearchString("=", $this->BLOOD_TYPE_ID->CurrentValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->BLOOD_TYPE_ID->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->BLOOD_TYPE_ID->EditValue = $arwrk;
            }
            ...
            if ($curVal != "") {
                ...
                $this->FAMILY_STATUS_ID->ViewValue = $this->FAMILY_STATUS_ID->Lookup !== null && is_array($this->FAMILY_STATUS_ID->Lookup->Options) ? $curVal : null;
            }
            if ($this->FAMILY_STATUS_ID->ViewValue !== null) { // Load from cache
                $this->FAMILY_STATUS_ID->EditValue = array_values($this->FAMILY_STATUS_ID->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[FAMILY_STATUS_ID]" . SearchString("=", $this->FAMILY_STATUS_ID->CurrentValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->FAMILY_STATUS_ID->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->FAMILY_STATUS_ID->EditValue = $arwrk;
            }
            ...
            if ($curVal != "") {
                ...
                $this->AKTIF->ViewValue = $this->AKTIF->Lookup !== null && is_array($this->AKTIF->Lookup->Options) ? $curVal : null;
            }
            if ($this->AKTIF->ViewValue !== null) { // Load from cache
                $this->AKTIF->EditValue = array_values($this->AKTIF->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[STATUS_PESERTA_KODE]" . SearchString("=", $this->AKTIF->CurrentValue, DATATYPE_STRING, "");
                }
                $sqlWrk = $this->AKTIF->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->AKTIF->EditValue = $arwrk;
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->STATUS_PASIEN_ID->Required) {
            if ($this->STATUS_PASIEN_ID->FormValue == "") {
        ...
        if ($this->BLOOD_TYPE_ID->Required) {
            if (!$this->BLOOD_TYPE_ID->IsDetailKey && EmptyValue($this->BLOOD_TYPE_ID->FormValue)) {
                $this->BLOOD_TYPE_ID->addErrorMessage(str_replace("%s", $this->BLOOD_TYPE_ID->caption(), $this->BLOOD_TYPE_ID->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // DATE_OF_BIRTH
        $this->DATE_OF_BIRTH->setDbValueDef($rsnew, UnFormatDateTime($this->DATE_OF_BIRTH->CurrentValue, 11), null, false);
        ...
        // BLOOD_TYPE_ID
        $this->BLOOD_TYPE_ID->setDbValueDef($rsnew, $this->BLOOD_TYPE_ID->CurrentValue, null, false);
        ...
        // MEDICAL_NOTES
        $this->MEDICAL_NOTES->setDbValueDef($rsnew, $this->MEDICAL_NOTES->CurrentValue, null, false);

        // newapp
        $this->newapp->CurrentValue = CurrentOrgId();
        $this->newapp->setDbValueDef($rsnew, $this->newapp->CurrentValue, null);
