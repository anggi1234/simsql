...
class PasienVisitationSearch extends PasienVisitation
{
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                $this->RESPONSIBLE_ID->AdvancedSearch->ViewValue = $this->RESPONSIBLE_ID->lookupCacheOption($curVal);
            } else {
                $this->RESPONSIBLE_ID->AdvancedSearch->ViewValue = $this->RESPONSIBLE_ID->Lookup !== null && is_array($this->RESPONSIBLE_ID->Lookup->Options) ? $curVal : null;
            }
            if ($this->RESPONSIBLE_ID->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->RESPONSIBLE_ID->EditValue = array_values($this->RESPONSIBLE_ID->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[RESPONSIBLE_ID]" . SearchString("=", $this->RESPONSIBLE_ID->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->RESPONSIBLE_ID->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->RESPONSIBLE_ID->EditValue = $arwrk;
            }
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckEuroDate($this->IN_DATE->AdvancedSearch->SearchValue)) {
        ...
        if (!CheckEuroDate($this->MODIFIED_DATE->AdvancedSearch->SearchValue)) {
