...
class PasienVisitationList extends PasienVisitation
{
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "PasienVisitationAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
    ...
    public function renderListOptions()
    {
        ...
        if ($Security->allowList(CurrentProjectID() . 'TREATMENT_BILL')) {
            ...
            $body .= "&nbsp;" . str_replace("%c", Container("TREATMENT_BILL")->Count, $Language->phrase("DetailCount"));
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("TreatmentBillList?" . Config("TABLE_SHOW_MASTER") . "=PASIEN_VISITATION&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_TRANS_ID", $this->TRANS_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue) . "&" . GetForeignKeyUrl("fk_DIANTAR_OLEH", $this->DIANTAR_OLEH->CurrentValue) . "&" . GetForeignKeyUrl("fk_VISITOR_ADDRESS", $this->VISITOR_ADDRESS->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("TreatmentBillGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'PASIEN_VISITATION')) {
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'PASIEN_VISITATION')) {
        ...
        if ($Security->allowList(CurrentProjectID() . 'TREATMENT_AKOMODASI')) {
            ...
            $body .= "&nbsp;" . str_replace("%c", Container("TREATMENT_AKOMODASI")->Count, $Language->phrase("DetailCount"));
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("TreatmentAkomodasiList?" . Config("TABLE_SHOW_MASTER") . "=PASIEN_VISITATION&" . GetForeignKeyUrl("fk_NO_REGISTRATION", $this->NO_REGISTRATION->CurrentValue) . "&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "&" . GetForeignKeyUrl("fk_DIANTAR_OLEH", $this->DIANTAR_OLEH->CurrentValue) . "&" . GetForeignKeyUrl("fk_VISITOR_ADDRESS", $this->VISITOR_ADDRESS->CurrentValue) . "&" . GetForeignKeyUrl("fk_TRANS_ID", $this->TRANS_ID->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                ...
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'PASIEN_VISITATION') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $detailTbl = Container("TREATMENT_BILL");
        $detailFilter = $detailTbl->sqlDetailFilter_PASIEN_VISITATION();
        $detailFilter = str_replace("@VISIT_ID@", AdjustSql($this->VISIT_ID->DbValue, "DB"), $detailFilter);
        $detailFilter = str_replace("@TRANS_ID@", AdjustSql($this->TRANS_ID->DbValue, "DB"), $detailFilter);
        $detailFilter = str_replace("@NO_REGISTRATION@", AdjustSql($this->NO_REGISTRATION->DbValue, "DB"), $detailFilter);
        $detailFilter = str_replace("@THENAME@", AdjustSql($this->DIANTAR_OLEH->DbValue, "DB"), $detailFilter);
        $detailFilter = str_replace("@THEADDRESS@", AdjustSql($this->VISITOR_ADDRESS->DbValue, "DB"), $detailFilter);
        $detailTbl->setCurrentMasterTable("PASIEN_VISITATION");
        $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
        $detailTbl->Count = $detailTbl->loadRecordCount($detailFilter);
        $detailTbl = Container("TREATMENT_AKOMODASI");
        $detailFilter = $detailTbl->sqlDetailFilter_PASIEN_VISITATION();
        $detailFilter = str_replace("@NO_REGISTRATION@", AdjustSql($this->NO_REGISTRATION->DbValue, "DB"), $detailFilter);
        $detailFilter = str_replace("@VISIT_ID@", AdjustSql($this->VISIT_ID->DbValue, "DB"), $detailFilter);
        $detailFilter = str_replace("@THENAME@", AdjustSql($this->DIANTAR_OLEH->DbValue, "DB"), $detailFilter);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fPASIEN_VISITATIONlist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fPASIEN_VISITATIONlist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fPASIEN_VISITATIONlist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_PASIEN_VISITATION" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_PASIEN_VISITATION\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fPASIEN_VISITATIONlist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
