 ...
 * Page class
 */
class VLaboratoriumGrid extends VLaboratorium
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VLaboratoriumGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fV_LABORATORIUMgrid";
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetall") {
                ...
                $this->DbDetailFilter = "";
                        ...
                        $this->DIANTAR_OLEH->setSessionValue("");
                        $this->GENDER->setSessionValue("");
                        $this->VISITOR_ADDRESS->setSessionValue("");
            }
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->CLINIC_ID->CurrentValue = "P013";
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            $keyValue = $rsnew['DIANTAR_OLEH'] ?? $rsold['DIANTAR_OLEH'];
            if (strval($keyValue) != "") {
                $masterFilter = str_replace("@DIANTAR_OLEH@", AdjustSql($keyValue), $masterFilter);
            } else {
                $validMasterRecord = false;
            }
            $keyValue = $rsnew['GENDER'] ?? $rsold['GENDER'];
            if (strval($keyValue) != "") {
                $masterFilter = str_replace("@GENDER@", AdjustSql($keyValue), $masterFilter);
            } else {
                $validMasterRecord = false;
            }
            $keyValue = $rsnew['VISITOR_ADDRESS'] ?? $rsold['VISITOR_ADDRESS'];
            if (strval($keyValue) != "") {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "PASIEN_VISITATION") {
            ...
            $this->DIANTAR_OLEH->CurrentValue = $this->DIANTAR_OLEH->getSessionValue();
            $this->GENDER->CurrentValue = $this->GENDER->getSessionValue();
            $this->VISITOR_ADDRESS->CurrentValue = $this->VISITOR_ADDRESS->getSessionValue();
        }
        ...
        if ($this->DIANTAR_OLEH->getSessionValue() != "") {
        $masterFilter = str_replace("@DIANTAR_OLEH@", AdjustSql($this->DIANTAR_OLEH->getSessionValue(), "DB"), $masterFilter);
        } else {
            $validMasterRecord = false;
        }
        if (strval($this->GENDER->CurrentValue) != "") {
            $masterFilter = str_replace("@GENDER@", AdjustSql($this->GENDER->CurrentValue, "DB"), $masterFilter);
        } else {
            $validMasterRecord = false;
        }
        if ($this->VISITOR_ADDRESS->getSessionValue() != "") {
        $masterFilter = str_replace("@VISITOR_ADDRESS@", AdjustSql($this->VISITOR_ADDRESS->getSessionValue(), "DB"), $masterFilter);
        } else {
            $validMasterRecord = false;
        }
        if ($validMasterRecord) {
        ...
        // DIANTAR_OLEH
        if ($this->DIANTAR_OLEH->getSessionValue() != "") {
            $rsnew['DIANTAR_OLEH'] = $this->DIANTAR_OLEH->getSessionValue();
        }

        // VISITOR_ADDRESS
        if ($this->VISITOR_ADDRESS->getSessionValue() != "") {
            $rsnew['VISITOR_ADDRESS'] = $this->VISITOR_ADDRESS->getSessionValue();
        }
