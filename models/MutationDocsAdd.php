...
class MutationDocsAdd extends MutationDocs
{
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    $returnUrl = 'MutationDocsEdit/'.urlencode(CurrentPage()->ID->CurrentValue).'?showdetail=GOOD_GF';
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->ORDER_VALUE->CurrentValue = "0";
        $this->MUTATION_VALUE->CurrentValue = "0";
        ...
        $this->ACKNOWLEDGEBY->CurrentValue = "-";
        $this->COMPANY_ID->CurrentValue = "1";
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'MUTATION_BY' first before field var 'x_MUTATION_BY'
        $val = $CurrentForm->hasValue("MUTATION_BY") ? $CurrentForm->getValue("MUTATION_BY") : $CurrentForm->getValue("x_MUTATION_BY");
        if (!$this->MUTATION_BY->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->MUTATION_BY->Visible = false; // Disable update for API request
            } else {
                $this->MUTATION_BY->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->MUTATION_DATE->CurrentValue = UnFormatDateTime($this->MUTATION_DATE->CurrentValue, 11);
        $this->MUTATION_BY->CurrentValue = $this->MUTATION_BY->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->CLINIC_ID_TO->ViewValue !== null) { // Load from cache
                ...
                $lookupFilter = $lookupFilter->bindTo($this);
                $sqlWrk = $this->CLINIC_ID_TO->Lookup->getSql(true, $filterWrk, $lookupFilter, $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
            ...
            $this->MUTATION_BY->EditValue = HtmlEncode($this->MUTATION_BY->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->MUTATION_BY->Required) {
            if (!$this->MUTATION_BY->IsDetailKey && EmptyValue($this->MUTATION_BY->FormValue)) {
                $this->MUTATION_BY->addErrorMessage(str_replace("%s", $this->MUTATION_BY->caption(), $this->MUTATION_BY->RequiredErrorMessage));
            }
        }
        ...
        if (in_array("GOOD_GF", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // MUTATION_BY
        $this->MUTATION_BY->setDbValueDef($rsnew, $this->MUTATION_BY->CurrentValue, null, false);
        ...
        if ($addRow) {
            ...
            $detailPage = Container("GoodGfGrid");
            if (in_array("GOOD_GF", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->ROOMS_ID->setSessionValue($this->CLINIC_ID_TO->CurrentValue); // Set master key
                $detailPage->ORG_ID->setSessionValue($this->CLINIC_ID_TO->CurrentValue); // Set master key
                $detailPage->FROM_ROOMS_ID->setSessionValue($this->CLINIC_ID->CurrentValue); // Set master key
                $detailPage->DOC_NO->setSessionValue($this->DOC_NO->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "GOOD_GF"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->ROOMS_ID->setSessionValue(""); // Clear master key if insert failed
                $detailPage->ORG_ID->setSessionValue(""); // Clear master key if insert failed
                $detailPage->FROM_ROOMS_ID->setSessionValue(""); // Clear master key if insert failed
                $detailPage->DOC_NO->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
